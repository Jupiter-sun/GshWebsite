				...
				if(typeof update_active === 'function') {
					...
					if(link_element.length > 0) {
						...
						if(nav.length > 0) {
							...
							var active_li = link_element.closest('li').addClass('active').parents('.nav li').addClass('active core');
...
(function($ , undefined) {
	...
	function Sidebar(sidebar, options) {
		...
		this.$sidebar.on(ace.click_event+'.ace.submenu', '.nav-list', function (ev) {
			...
			//if not core and visible, let's core it and make it visible
			if( sub_hidden ) {//being shown now
			  $(parent_ul).find('> .core > .submenu').each(function() {
				//close all other core submenus except for the active one
		...
		this.scroll_to_active = function() {
			...
			try {
				...
				if(vars['minimized'] && !vars['collapsible']) {
					...
					if($active.length == 0)	$active = $nav.find('.active:not(.core)')
				}
